...
| --- | --- | --- |
| [circleci](#circleci) | `http` | Query the current user, recent pipelines, pipeline workflows, and workflow jobs from CircleCI API v2. |
...
`CIRCLECI_API_BASE` (optional)<br />
default `https://circleci.com/api/v2`

Base URL for the CircleCI API v2.

`CIRCLECI_TOKEN` (required)

Create a CircleCI API token with access to the organizations and projects
you want to query.
